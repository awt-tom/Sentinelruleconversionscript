...
if (-not (Get-Module -ListAvailable -Name SentinelARConverter)) {
    Write-Host "SentinelARConverter module not found. Installing super awesome module from the PowerShell Gallery..."
...
foreach ($solution in $selectedSolutions) {
    ...
    foreach ($item in $yamlFileNames) {
        try {
            ...
            $destinationFolderPath = Join-Path $destinationRoot $relativeFolderPath
